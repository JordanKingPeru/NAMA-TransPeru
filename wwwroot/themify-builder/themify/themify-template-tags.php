	    ...
	    if ('image' === $type) {
            $html .= themify_get_image(
                array(
                    ...
                    'alt'=> $site_name,'disable_lazy'=>Themify_Enqueue_Assets::$isFooter === false,
...
if (!function_exists('themify_post_content')){
	...
	<div class="entry-content<?php if($iseditable===true && $themify->display_content==='content'):?> tb_editor_enable<?php endif;?>"<?php if($iseditable===true):?> data-type="<?php echo $themify->display_content?>" contenteditable="false"<?php endif;?>>
		...
		<?php themify_before_post_content(); ?>
		<?php the_excerpt(); ?>
		<?php themify_after_post_content(); ?>
...
if (!function_exists('themify_comments')) {
    ...
    function themify_comments() {
		...
		<div id="comments" class="commentwrap tf_clearfix">
            ...
            elseif(true===$hasComment && themify_is_themify_theme() && is_file(Themify_Enqueue_Assets::$THEME_CSS_MODULES_URI.'comments.css')){
...
function themify_menu_nav( $args = array(),$reinit=false ) {
    ...
    $cacheDisable=$reinit===true  || themify_isDevMode() || TFCache::get_cache_plugins('any') || themify_check('setting-cache-menu',true) || themify_check('setting-cache-html',true) || defined( 'POLYLANG_VERSION' );    
    ...
    if($cacheDisable===false){
		...
		if ( class_exists( 'WPML_LS_Render' ) ) {
