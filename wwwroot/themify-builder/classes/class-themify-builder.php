...
if (!class_exists('Themify_Builder')) :
	...
	private static $async = array('word-count','shortcode');
        ...
        public function wp_loaded() {
            ...
            if(($is_admin===true && Themify_Builder_Model::hasAccess()) || Themify_Builder_Model::is_frontend_editor_page()) {
                ...
                if($is_active===true) {
                    ...
                    } elseif ( empty( $post_type ) || (! empty( $post_type ) && ! Themify_Builder_Model::is_builder_disabled_for_post_type( $post_type )) ) {
                        // Builder write panel
                        if ( $is_admin === true ) {
                            // Filtered post types
	...
	private function load_frontend_interface() {
		...
		wp_enqueue_style('themify-builder-inline-editor', themify_enque($editorUrl . 'inline-editor.css'), false, THEMIFY_VERSION);
		...
		switch ($script) {
		    ...
		    case 'themify-colorpicker':
			    wp_enqueue_script($script, themify_enque(THEMIFY_METABOX_URI . 'js/themify.minicolors.js'), null, THEMIFY_VERSION, true); // grab from themify framework
			...
			wp_enqueue_script('themify-builder-inline-editing', themify_enque($frontendUrl . 'themify-builder-inline-editing.js'), array('themify-builder-front-ui-js'), THEMIFY_VERSION, true);
	...
	public static function get_i18n() {
		...
		'image_help2'=>__('Image can not be cropped because it is on content', 'themify'),
	...
	private function load_admin_interface() {
		...
		wp_enqueue_style('themify-backend-ui', themify_enque($editorUrl . 'backend/backend-ui.css'), null, THEMIFY_VERSION);
	...
	public function footer_js() {
	    ...
	    Themify_Enqueue_Assets::localize_script('themify-main-script', 'tbLocalScript',$options);
	}
	...
	public function clear_static_content( $content ) {
		...
		/* for post types that don't have Builder, static content should be displayed */
		if ( Themify_Builder_Model::is_builder_disabled_for_post_type( get_post_type() ) ) {
			return $content;
		}
	...
	public function builder_show_on_front($content) {
		...
		elseif (in_array($post_id, self::$post_ids, true) || (strpos( $post->post_content, '<!--more-->' )!==false && ! is_single( $post->ID ) && !is_page( $post->ID ) )) {
	    ...
	    self::$post_ids = array_unique(self::$post_ids);
	    if (array_shift(self::$post_ids) === $post_id) {
	...
	public function inline_css(){
		...
		if($is_lax!==false){
			...
			if($is_lax==='m'){
				...
				$p.='@media screen and (max-width:'.($tablet+2).'px){.themify_builder .tf_lax_done{opacity:unset!important;transform:unset!important;filter:unset!important;backface-visibility:unset!important}}';
			}
			$st.=$p;
		}
			if($is_sticky!==false){
        ...
        $visiblity_st='';
        $p=$is_builder_active===true?'display:none!important':'width:0!important;height:0!important;padding:0!important;visibility:hidden!important;margin:0!important;display:table-column!important';
        foreach($bp as $k=>$v){
