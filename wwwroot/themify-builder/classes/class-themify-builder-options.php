...
class Themify_Builder_Options {
	...
	function builder_lightbox(){
		...
		?>
		<select class="tb_gallery_lightbox" name="<?php echo esc_attr( $this->option_name . '['.$this->current_tab.'_lightbox]' ); ?>">
			<option value="enable" <?php selected( $selected, 'enable'); ?>><?php _e('Enable', 'themify') ?></option>
			<option value="disable" <?php selected( $selected, 'disable'); ?>><?php _e('Disable', 'themify') ?></option>
		</select>
		<?php
	...
	function load_enqueue_scripts( $page ) {
		if ( 'toplevel_page_themify-builder' === $page ) {
			...
			wp_enqueue_script( 'themify-builder-admin-settings', themify_enque(THEMIFY_BUILDER_URI . '/js/plugin/themify-builder-admin-settings.js'), array('jquery'), THEMIFY_VERSION, true );
	...
	function builder_maintenance_field() {
		...
		$output = '
			<select id="tools_maintenance_mode" name="' . $this->option_name . '[tools_maintenance_mode]">
		...
		$output .= '<textarea id="tools_maintenance_message" name="' . $this->option_name . '[tools_maintenance_message]" class="width10">' . esc_html( $message ) . '</textarea>';
		...
		$output .= sprintf('<br/><div class="tb_maintenance_page"><select id="%s" name="%s">%s<option>%s</option></select><p class="description">%s</p></div>',
			...
			$this->option_name . '[tools_maintenance_page]' ,
